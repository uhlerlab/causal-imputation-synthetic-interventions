...
def fill_tensor(pre_df, post_df, t=0.99, center=True, rcond=1e-15, alpha=0.05, include_pre=True, \
    ...
    for iv in tqdm(pairs.keys()):
        ...
        for unit in pairs[iv]:
            ...
            X1_post = post_df.loc[(post_df.unit.isin(donor_units)) & (post_df.intervention.isin(donor_ivs)) & ~(
            ...
            # get post-intervention data from donor therapies
            X2_post = post_df.loc[
                (post_df.unit == unit) & (post_df.intervention.isin(donor_ivs)) & ~(post_df.intervention == iv)]
            ...
            yh = hsvt_ols(X1, X2, y1, t=t, center=center, rcond=rcond, include_pre=include_pre, method=rank_method,
